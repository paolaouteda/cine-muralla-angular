...
export class AdministradorComponent implements OnInit {
  ...
  constructor(
    ...
    private mapasService: MapaButacasServices,
  ...
  ngOnInit() {
    ...
    this.salasService
      .getTipoRepro()
      .subscribe(data => (this.reproducciones = data));
    ...
    this.peliculasService
      .getSubtitulos()
      .subscribe(data => (this.subtitulos = data));
  }
  ...
  crearPelicula() {
    const pelicula = {
      ...
      imaVertical: this.imaVertical,
      imaHorizontal: this.imaHorizontal
    };
  ...
  escogerSalas() {
    this.salasService
      .getIdSala(this.nombreSala)
      .subscribe(data => (this.idSalas = data));
  }
  ...
  crearMapaButaca() {
    ...
    let entrada = "ID";
    let pasillo = "ICD";
    if (!this.salidaDerecha) salida = salida.replace("D", "");
    if (!this.salidaIzquierda) salida = salida.replace("I", "");
    if (!this.salidaCentro) salida = salida.replace("C", "");
    if (!this.pasilloDerecho) pasillo = pasillo.replace("D", "");
    if (!this.pasilloIzquierdo) pasillo = pasillo.replace("I", "");
    if (!this.entradaDerecha) entrada = entrada.replace("D", "");
    if (!this.entradaIzquierda) entrada = entrada.replace("I", "");
    if (!this.pasilloCentro) pasillo = pasillo.replace("C", "");
    else {
